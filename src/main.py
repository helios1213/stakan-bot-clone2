...
async def db_prune_loop(db_path: str, live_db_path: str, research_path: str, interval_sec: int = 86400) -> None:
    ...
    #
    # `signal_features` — НАЙБІЛЬША таблиця проєкту і єдина, що містить ПОВНИЙ
    # кандидатний простір: не лише сигнали, на яких ми діяли, а й ВІДКИНУТІ,
    # з `rejected_by` і форвардними доходностями по кожному. Тобто це єдине
    # джерело контрфактики «скільки принесло б те, що ми пропустили»
    # (`live_trades`/`shadow_trades` містять тільки те, на чому діяли).
    #
    # 30 -> 14 ДНІВ, РІШЕННЯ ОПЕРАТОРА 2026-09-02. Виміряно на цей день:
    # 20.3 млн рядків / 3.64 ГБ за 19 накопичених днів = ~1.07 млн рядків і
    # ~190 МБ на добу, 192 байти на рядок. 30 днів давали б ≈5.7 ГБ сталого
    # стану при 2.3 ГБ вільних на диску — тобто файл заповнив би диск сам
    # приблизно за 11 днів. 14 днів фіксують розмір на ≈2.7 ГБ.
    #
    # УВАГА: prune робить лише DELETE + wal_checkpoint, БЕЗ VACUUM. Тобто
    # після зміни ретенції місце на диску НЕ звільниться саме — сторінки
    # підуть у freelist, а файл лишиться тим самим. Потрібен окремий VACUUM
    # при зупиненому боті (auto_vacuum=0, тож incremental не працює).
    RET_RESEARCH = [
        ("signal_features",          "ts",     14 * 86400),
