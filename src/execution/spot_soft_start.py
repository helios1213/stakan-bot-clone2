...
class SoftStartConfig:
    ...
    # 0 -> 1 (2026-09-15, оператор: «клон 1 слот 2 — тільки фʼючі без спота»). План розіграв buys=0 sells=12,
    # а після розчистки перед кампанією монет на слоті немає — продавати нічого, купувати план не дає,
    # і спот стояв цілу добу. Із 40 денних планів трьох ботів так випало 1 раз (≈1/26 на добу).
    buys_per_day_min: int = 1
