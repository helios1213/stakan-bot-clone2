...
class SoftStartConfig:
    ...
    # 0 -> 1 (2026-09-15) -> 4 (2026-09-16, рішення оператора «бідний фарм на клоні 1»).
    # Спершу нуль давав добу зовсім без спота; далі виявилось, що і 1-3 купівлі на добу читаються як
    # простій. Виміряно на 20 тис. розіграшів: мінімум 4 і вага від 0.35 дають медіану 12 купівель на добу
    # (було 9) і 10% найтихіших днів не нижче 6 (було 3). Тихі дні лишаються — зникають лише порожні.
    buys_per_day_min: int = 4
    buys_per_day_max: int = 25
    sells_per_day_min: int = 0
    sells_per_day_max: int = 20
    order_usdt_min: float = 1.6          # >= MIN_HOLD_USDT з запасом на перетин книги/комісію (див. нижче)
...
# НАЙМЕНША ПОЗИЦІЯ МОНЕТИ, ЯКУ ПРОГРІВ ЛИШАЄ НА СПОТІ (рішення оператора 2026-09-15).
# Монета після дій прогріву — або 0, або >= 1.5 USDT. Залишок трохи вище біржового мінімуму 1.10
# після невеликого просідання ціни стає пилом, який уже не продати: клон 2 15.09 лишився з SUI 0.71.
# Тому: купівля не менша за 1.6 (1.5 + перетин книги 0.2% + комісія + округлення кількості), базовий
# залишок при звичайних продажах не нижче 1.5, а розпродаж продає монету ПОВНІСТЮ, якщо частковий
# продаж лишив би менше 1.5.
MIN_HOLD_USDT = 1.5
MIN_BUY_USDT = 1.6
...
БЕЗ ДРІБНИХ ЗАЛИШКІВ (і в розчистці перед кампанією, і в кінцевому розпродажі): якщо після часткового
        продажу на монеті лишилось би менше `MIN_HOLD_USDT`, монета продається ПОВНІСТЮ цим же ордером.
        Інакше залишок стає пилом, який потім не продати: primary слот 2, 2026-09-14 — перший тік продав 59%
        кожної монети, лишилось TRX 0.84 і LINK 0.70 USDT, а наступний тік пропустив обидві як «нижче
        мінімуму 1.10». До 2026-09-15 правило діяло лише в розчистці і з порогом 1.10.
        """
        cfg = self.cfg
        sent = 0
        ...
        for token, symbol, held, px, value, cur in holdings:
            ...
            full = False
            if value - sell_value < MIN_HOLD_USDT:
