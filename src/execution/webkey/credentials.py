...
#
# ДВА ВІДКАТИ, обидва без зміни коду — і вони РІЗНІ:
#   MEXC_CHASH=<hex>   — підмінити САМЕ chash, лишивши поточний набір полів p0.
#                        Перебиває і знімок, і значення, привезене з сервера.
#   MEXC_DOLOS_LEGACY=1 — повернути ПАРУ «старий chash + поля ордера» цілком.
#                        Саме її пройшли 16 932 ордери.
#
# УВАГА, це вже одного разу розійшлось із дійсністю: коли chash переїхав у
# живий конфіг (`dolos_config.CACHE`), `MEXC_CHASH` став читатись лише тут і
# на дріт не потрапляв ЗОВСІМ — інструкція лишалась, важіль зник. Тепер
# override застосовується в `_DolosRuntime.as_signing_dict`, тобто в тому
# самому місці, звідки береться значення для підпису.
...
# Порожньо = «не задано». Саме ПОРОЖНЄ, а не дефолт: споживачу треба знати,
# чи оператор задав значення явно, бо мовчазний дефолт нічого не перебиває.
CHASH_ENV_OVERRIDE = os.environ.get("MEXC_CHASH", "").strip()
BOOTSTRAP_CHASH = CHASH_ENV_OVERRIDE or _BOOTSTRAP_CHASH_DEFAULT
        ...
        await self.db.execute(
            ...
            UPDATE webkey_slots
               SET webkey_blob = ?,
                   ...
                   last_error = NULL,
                   -- Базова лінія відновлення описує КОНКРЕТНИЙ акаунт: вона
                   -- армиться з його накопиченого PnL. Новий ключ = інший
                   -- акаунт, тож стара ціль стосується вже неіснуючого.
                   -- Спіймано 2026-08-05: ключ від 08-04, база армлена 07-21,
                   -- ціль $42.47 при реальних -$537.40. NULL = монітор
        ...
        await self.db.execute(
            ...
            UPDATE webkey_slots
               SET webkey_blob=NULL,
                   ...
                   open_throttle_until=NULL,
                   -- Повне очищення слота від засувок/помилок: +recovery
                   -- (last_error/open_throttle вже вище).
                   recovery_mode=0,
                   recovery_baseline_ts=NULL,
                   recovery_target_usdt=NULL,
                   updated_at=?
