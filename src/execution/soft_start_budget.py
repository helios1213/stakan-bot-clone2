...
#
# ЧОМУ САМЕ 10, А НЕ 5 (історія). Розмір ордера рахується як 12% балансу, а
# мінімальний ноціонал на споті MEXC ~1 USDT. При балансі 5 стеля розміру була б
# 0.60 — нижче мінімуму, і КОЖЕН ордер відхилявся б біржею.
#
# 10 -> 20, РІШЕННЯ ОПЕРАТОРА 2026-09-02. Поріг застосовується до КОЖНОГО
# майданчика ОКРЕМО (спот і фʼючерси судяться незалежно, див.
# `soft_start_runner.start()`), тому це означає «20 на споті І 20 на
# фʼючерсах = 40 разом» — саме так це й було сформульовано.
#
# ЩО ЦЕ ДАЄ ЗА ВИМІРЯНИМИ ЧИСЛАМИ. При 10 спотовий ордер виходив 1.10-1.20 при
# мінімальному ноціоналі біржі 1.1 — тобто запасу практично нуль, і будь-який
# токен із вищим мінімумом давав серію відмов. При 20 ордер 1.10-2.40, а
# фʼючерсна маржа 6-14% дає 1.20-2.80 замість 0.60-1.40, тож більше пар
# юніверсу проходить перевірку `affordable` (35% гаманця) на низькому плечі.
