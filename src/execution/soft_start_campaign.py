...
# СКІЛЬКИ ЛИШАЄТЬСЯ В МОНЕТАХ ПІСЛЯ КАМПАНІЇ — діапазон, а не число
# (рішення оператора 2026-09-05). Було рівно 0.20, і саме тому це помітно:
# рахунок, який після КОЖНОЇ кампанії й на КОЖНОМУ акаунті лишається з
# точно тією самою часткою в монетах, — це підпис. Уся решта прогріву
# рандомізована між акаунтами (набір токенів, квоти, вага дня, розміри),
# ця частка лишалась єдиною константою.
#
# Розігрується РАЗ НА КАМПАНІЮ і зберігається — як `day_weights`. Інакше
# кожен прохід розпродажу (їх до MAX_WIND_DOWN_PASSES) цілив би в іншу
# частку, і послідовність проходів перестала б сходитись.
WIND_DOWN_KEEP_MIN = 0.15
WIND_DOWN_KEEP_MAX = 0.25
...
class CampaignState:
    ...
    account_key: str = ""
    # РОЗЧИСТКА СПОТА: до якої миті монети мають бути злиті, і чи вже злиті.
    #
    ...
    # баланс ЧИННОЇ кампанії. Розчистка спота вмикає ЛИШЕ `start_if_new()` —
    # тобто момент, коли кампанія справді починається.
    #
    # Початок вікна не зберігаємо окремо: він дорівнює `started_at`, бо
    # кампанія і розчистка спота озброюються одним викликом.
    preclear_until: float = 0.0
    preclear_done: bool = True
    # Частка спотового балансу, що лишається в монетах у кінці цієї
    # кампанії. 0.0 = ще не розіграно (так виглядають файли попередніх
    # версій) — тоді її розіграють ліниво при першому ж розпродажі.
    wind_down_keep: float = 0.0
...
class SoftStartCampaign:
    ...
    def start_if_new(self, account_key: str = "") -> bool:
        ...
        self.state = CampaignState(days=days, started_at=_now,
                                   ...
                                   preclear_done=False)
        self._save()
        logger.info("soft-start campaign: старт (%s), %d доби; розчистку спота "
                    "розмазано на %.0f хв", why, days, _window / 60)
        return True

    # ---- розчистка спота перед стартом --------------------------------------

    def wind_down_keep(self) -> float:
        """Яку частку СПОТОВОГО БАЛАНСУ лишити в монетах наприкінці кампанії.

        [0.15; 0.25], розіграно РАЗ НА КАМПАНІЮ і збережено — тими самими
        міркуваннями, що й `day_weight`: рестарт не має перерозігрувати число,
        інакше кожен прохід розпродажу цілив би в іншу частку.

        РАХУЄТЬСЯ ВІД СПОТОВОГО БАЛАНСУ РАЗОМ (монети + вільний USDT), а не
        від вартості монет і не від кожної монети окремо — це три різні числа.
        Перша версія рахувала по кожній монеті й лишала 3.46 USDT там, де
        правило оператора дає 5.09, бо USDT у знаменник не входив.

        Ліниво: у файлів попередніх версій поля немає, тож кампанія, що вже
        триває, отримає своє число при першому розпродажі, а не лишиться на
        старій константі 0.20.
        """
        if not self.state.wind_down_keep:
            self.state.wind_down_keep = round(
                self.rng.uniform(WIND_DOWN_KEEP_MIN, WIND_DOWN_KEEP_MAX), 3)
            self._save()
            logger.info("soft-start campaign: у монетах лишиться %.1f%% "
                        "спотового балансу", self.state.wind_down_keep * 100)
        return self.state.wind_down_keep
