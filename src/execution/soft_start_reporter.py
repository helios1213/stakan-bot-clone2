...
class SoftStartReporter:
    ...
    def render_final(self, reason: str, *, spent: float = 0.0,
        ...
        lines += [
            ...
            # ЗНАК ЄДИНИЙ: мінус = гроші пішли з гаманця. Раніше комісії йшли
            # з «+», а PnL з «-», і читач мусив тримати в голові дві різні
            # конвенції в сусідніх рядках.
        ...
        if not spot_pnl and held_value:
            # НУЛЬ ТУТ БУВАЄ ЧЕСНИЙ, і його треба відрізняти від «не рахували».
            # Монети, куплені до появи обліку за собівартістю, лежать у
            # legacy-відрі: їхньої ціни купівлі ми не знаємо, тож продаж із
            # нього дає нуль. Без цього рядка «спот +0.0000» читається як
            # «спот нічого не коштував», а насправді ми просто не міряли.
            lines.append("<i>спотовий PnL по монетах, куплених до появи "
                         "обліку за собівартістю, не рахується — там нуль "
                         "означає «не виміряно», а не «без результату».</i>")
        lines.append("<code>" + "─" * 30 + "</code>")
        # ОДНЕ ЗРОЗУМІЛЕ ЧИСЛО БЕЗ ЗНАКА-ПАСТКИ. Було «РАЗОМ +4.8056», і плюс
        # читався як заробіток, хоча це ВИТРАТА. Тепер підсумок називається
        # тим, чим є, а знак не треба розшифровувати.
        if net > 0.05:
            lines.append(f"<b>Прогрів обійшовся: {net:.2f} USDT</b>")
        elif net < -0.05:
            lines.append(f"<b>Прогрів вийшов у плюс: {-net:.2f} USDT</b>")
        else:
            lines.append("<b>Прогрів вийшов приблизно в нуль</b>")
        if held_value:
    ...
    def render(self, *, day: int | None = None, days: int | None = None,
        ...
        if spent is not None and (futures_pnl is not None
            ...
            # Без знака-пастки: «разом +4.806» читалось як заробіток, хоча це
            # витрата. Кажемо словом, що саме сталось.
            if net > 0.005:
                meta.append(f"коштувало {net:.3f} USDT")
            elif net < -0.005:
                meta.append(f"у плюс {-net:.3f} USDT")
            else:
                meta.append("в нуль")
        if held_value:
