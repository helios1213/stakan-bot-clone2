...
## Current state (as of 2026-09-02) — ПРОГРІВ У ЛАЙВІ НА 4 СЛОТАХ, АРБ ВИМКНЕНО
...
```
**Прямий доказ, ЩО саме тиснуло** — у журналі primary: `Sep 04 18:31:26 session-19554.scope: Consumed 7min 40.242s CPU time, 635.9M memory peak, 484.1M memory swap peak.` Сесія на 636 МБ RSS + 484 МБ свопу на коробці з 955 МБ; завершилась о 18:31:26. Зараз на primary **жодного процесу node/claude немає** — оператор переніс роботу з Claude на srv2 (108.160.143.238).
**`signal_features` як «наступний підозрюваний» ВІДПАВ** — і його ж 04.09 УВІМКНЕНО на клоні, тобто тепер він на обох.
**Залишок, який сам не розсмокчеться:** після перезапуску сесії бот на primary усе одно сидів у свопі (RSS 39 МБ + swap 60 МБ проти 132 МБ RSS у клона) — сторінки, витиснені о 18:31, назад не фолтяться. Лікується РЕСТАРТОМ контейнера, і його зроблено 04.09 19:16 UTC (0 відкритих `live_trades` перед тим, дренаж чистий): стало RSS 111 МБ / swap 0. **Якщо після майбутнього піску памʼяті бот виглядає млявим — дивись `VmSwap` у `/proc/<pid>/status`, а не лише `free -m`.**
**Не закрито:** `vm.swappiness=60` на обох при 955 МБ RAM і боті, що пікує до 400 МБ — не чіпав, це рішення оператора. **Токен Telegram оператор мав ротувати — спитай, чи зробив.**
...
**`VACUUM` для research.db НЕ потрібен:** freelist на primary **0.1%** (3 МБ) — звільнені сторінки вже переспожиті вставками, файл щільний. На клоні 23.3%, але це 39 МБ.
...
```
- **Прогрів іде на ВСІХ ЧОТИРЬОХ слотах.** Арбітраж (`live_enabled`) вимкнено скрізь, live-пар нуль, тож `shadow_twin` не росте (primary 1677, клон 432).
...
```
**На клоні `signal_features` НЕМАЄ ВЗАГАЛІ** — там немає recorder-стека, тож ретенція там no-op, а research-база 0.12G, а не 4. Головна ціль стиснення на клоні була інша: `stakan.db` мав **46% мертвих сторінок**.
