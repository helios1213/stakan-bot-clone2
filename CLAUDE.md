...
- `src/execution/soft_start_reporter.py` — живе повідомлення в Telegram + фінальний звіт. З 2026-09-15 живе повідомлення має окремі блоки **Futures** і **Spot** (по 6 останніх, новіші зверху) і **Інше** (старт кампанії, попередження розчистки, 3 останні); спільного «Recent actions» більше немає.
...
- **Нижня межа активності доби (рішення оператора 2026-09-16):** `buys_per_day_min=4` у `spot_soft_start.py` і `SoftStartCampaign.WEIGHT_MIN=0.35`. Клон 1 отримав ваги 0.17 і 0.22/0.28 два дні поспіль -> плани «2 купівлі, 3 продажі» на добу, і прогрів читався як простій. Виміряно на 20 тис. розіграшів: медіана 12 купівель/добу (було 9), 10% найтихіших днів >= 6 (було 3), мінімум 4. Тихі дні лишаються, зникають лише порожні. Тести на обидві межі пінять ЧИСЛА, а не константи (звірка з константою давала тавтологію — спіймано мутантом).
