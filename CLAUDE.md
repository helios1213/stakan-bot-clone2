...
**LIVE ВИМКНЕНО оператором 2026-08-21 увечері** — обидва слоти `live_enabled=0`. Поки так, burst-детектор і `shadow_twin` не працюють: обидва живляться з `live_trades`.

**НАСТУПНИЙ КРОК — НЕ КОД, А ОЧІКУВАННЯ.** Треба назбирати `shadow_twin` (кілька сотень рядків) і реальні burst-алерти, щоб калібрувати `queue_frac` і пороги алерту НА ДАНИХ. Три невдалі ітерації з `mexc_feed_lag_ms` сталися саме через калібрування на 6-14 рядках.

**Як калібрувати `queue_frac`:** у `shadow_twin` порівняти `shadow_filled_pct` проти `live_filled_pct` на тих самих сигналах. Якщо shadow систематично повніший — знижувати.
...
>500мс    18       -$21.06   -2.99
```
Монотонно. Це **adverse selection у чистому вигляді**: швидкий філ = ліквідність була на місці; філ, що висів 500мс і все одно стався, означає, що **ціна прийшла до нас**, бо пішла проти. Відсічення на 350мс сьогодні прибрало б 29 угод і -$35.13. Це той самий «пост-вхідний канал», який аудит назвав найбільшим і без фікса — ось він із цифрами. **Фікса ще немає, це кандидат номер один.**
