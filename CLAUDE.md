...
**ВІДКРИТЕ:** диск primary **96% (1.4G вільно)** — це не фон, а тригер для §1.5/§2.5; чистка `data/` (5.2G) це рішення оператора. `_stop_attempts` як лічильник алертів тепер росте щополла, тож тротл `% 30` дає алерт раз на ~30 хв — не міняв.
