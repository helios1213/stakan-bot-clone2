...
## Current state (as of 2026-09-04 23:30 UTC) — ПРОГРІВ У ЛАЙВІ ЛИШЕ НА 2 СЛОТАХ КЛОНА, АРБ ВИМКНЕНО СКРІЗЬ
...
- **Прогрів (soft-start) ОЗБРОЄНИЙ живими грошима на ОБОХ ботах, але зараз ТОРГУЄ лише клон** (виміряно 04.09 23:29 UTC: змінна стоїть на обох, а кнопка 🌱 `soft_start_enabled` увімкнена лише на двох слотах КЛОНА — на primary 0/0). Озброєно двома гейтами: `SOFT_START_LIVE=1` у compose (читається ОДИН РАЗ на старті — перемикається лише через `up -d`) і кнопка 🌱 у `/slot N`. Роззброїти: прибрати змінну + `up -d`, але **лише коли позиція прогріву закрита**.
- **Арбітражна торгівля (`live_enabled`) ВИМКНЕНА** на всіх слотах. `shadow_twin` не росте.
- **КАМПАНІЇ ЙДУТЬ ЛИШЕ НА ДВОХ СЛОТАХ КЛОНА** (виміряно 04.09 23:29 UTC: `soft_start_enabled` = 0,0 на primary і 1,1 на клоні; `soft_start_campaign_slot{1,2}.json` — `"finished": true` на primary, `false` на клоні). **⏳ ЦЕЙ РЯДОК ПРОТУХАЄ 05.09 ~17:54 UTC** — саме тоді обидві кампанії клона добігають 3 діб і вимикають себе самі (старт 02.09 17:54 і 17:51 UTC). Після цього чекай `soft_start_enabled=0` на всіх чотирьох і фінальний звіт у Telegram; перевіряй, а не вір цьому рядку. Кожна триває 3 доби і вимикає себе сама; після завершення 🌱 можна натиснути знову — почнеться нова з чистим обліком. **Перед тим як щось роззброювати, ПЕРЕВІР стан**, а не вір цьому рядку: `SELECT slot_id, soft_start_enabled, live_enabled FROM webkey_slots` і `soft_start_campaign_slotN.json`. Роззброєння посеред живої кампанії осиротить фʼючерсну позицію або обнулить облік.
- **🌱 ТЕПЕР СПЕРШУ ПРОДАЄ МОНЕТИ, І ЛИШЕ ТОДІ ГРІЄ (2026-09-05, рішення оператора).** Будь-яка НОВА кампанія (перший запуск / повторне 🌱 після завершеної / зміна вебкея — усі три випадки `start_if_new`) озброює **розчистка спота**: `tick()` питає `campaign.preclear_pending()` ПЕРШИМ і, поки той True, не гріє взагалі. Монети зливаються `wind_down`-ом із `keep_frac`, що лінійно спадає 1.0 -> 0.0 за випадкове вікно **10-30 хв** (`PRECLEAR_MIN_SEC`/`PRECLEAR_MAX_SEC`) — одним викликом `keep_frac=0` це був би злив однією пачкою.
...
- **ВИДАЛЕННЯ АБО ПЕРЕКЛЕЮВАННЯ ВЕБКЕЯ ЗНІМАЄ ОБМЕЖЕННЯ СЛОТА (2026-09-09, рішення оператора).** `LiveExecutorPool.clear_slot_restrictions()` — ОДНА функція на три шляхи (майстер вставки, `/webkey_remove`, кнопка меню). Знімає: fee-guard халт (`_halted`, `_halt_was_preventive`, `_fee_probe_until`), `slot_level_error`, `account_block`, `last_error` у БД. Повідомлення каже, що саме знято, і мовчить, якщо нічого не стояло.
  ...
  **⚠️ ВЕБПАНЕЛЬ ЦЬОГО НЕ ВМІЄ:** її `POST /api/accounts/{n}/remove` ходить у БД повз процес бота і памʼять екзекутора не чистить. Видаляти ключ — через Telegram.
...
**Стан акаунтів (ВИМІРЯНО 02.09, оператор ротує акаунти — числа протухають разом із ключем):**
```
...
```
- **Прогрів іде лише на ДВОХ слотах КЛОНА** — на primary обидві кампанії завершились самі (`"finished": true`, `soft_start_enabled=0`; вимір 04.09 23:29 UTC). Арбітраж (`live_enabled`) вимкнено на всіх чотирьох, live-пар нуль, тож `shadow_twin` ЗАРАЗ не росте (primary **1779**, клон 432). **Число primary протухає щоразу, як вмикають лайв.**
