...
- **🌱 ТЕПЕР СПЕРШУ ПРОДАЄ МОНЕТИ, І ЛИШЕ ТОДІ ГРІЄ (2026-09-05, рішення оператора).** Будь-яка НОВА кампанія (перший запуск / повторне 🌱 після завершеної / зміна вебкея — усі три випадки `start_if_new`) озброює **передпродаж**: `tick()` питає `campaign.preclear_pending()` ПЕРШИМ і, поки той True, не гріє взагалі. Монети зливаються `wind_down`-ом із `keep_frac`, що лінійно спадає 1.0 -> 0.0 за випадкове вікно **10-30 хв** (`PRECLEAR_MIN_SEC`/`PRECLEAR_MAX_SEC`) — одним викликом `keep_frac=0` це був би злив однією пачкою.
...
**9. ПЕРЕДПРОДАЖ ПЕРЕД НОВОЮ КАМПАНІЄЮ (запит оператора).** Деталі поведінки — у `## Current state`. Тут те, що варте наступної сесії.
