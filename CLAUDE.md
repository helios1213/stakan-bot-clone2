...
**ВІДКРИТЕ:** коміти `d738bea` і наступний **ЗАКОМІЧЕНІ Й ЗАПУШЕНІ НА ОБИДВА, АЛЕ НЕ РОЗКОЧЕНІ** — рестарт відкладено, бо обидва боти тримають живі позиції з плечем 18-19x. Розкотити разом із `VACUUM`, коли позиції закриються. `_stop_attempts` як лічильник алертів тепер росте щополла, тож тротл `% 30` дає алерт раз на ~30 хв — не міняв.
